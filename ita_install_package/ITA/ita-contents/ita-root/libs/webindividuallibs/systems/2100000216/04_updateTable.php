...
//
//  【処理概要】
//    originalの04_updateTable.phpを元に
//    ・update対象キー(strNuberForRI)をinterger型以外も更新できるよう対応
//    ・DISUSE_FLAGの確認をskipするように変更
//
//////////////////////////////////////////////////////////////////////
...
function updateTableMain($intBaseMode, $strNumberForRI, $reqUpdateData=null, $strTCASRKey=null, $ordMode=0, &$aryVariant=array(), &$arySetting=array()) {
    ...
    // ---$strNumberForRI 選択した(更新ボタンを押した)シーケンス名(変数名がNumberなっているが実際はNumberではない)
    ...
    try{
        ...
        switch($intBaseMode) {
            case 1:
                ...
                //
                // ----更新対象レコードをSELECT
                $strNumberForRI = base64_decode($strNumberForRI);  // intBaseMode=1のときはbase64 encodeされているのでdecodeする
                $arrayResult = selectRowForUpdateWithoutNumericCheck($objTable, $strNumberForRI, $ordMode, 0);
...
function selectRowForUpdateWithoutNumericCheck (&$objTCA, $intRINo, $strOrdMode, $intMode=0) {
    global $g;
